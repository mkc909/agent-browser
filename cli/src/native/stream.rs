...
async fn handle_http_request(
    ...
    // Handle POST /api/chat (AI chat proxy)
    ...
    // Handle GET /api/models (list available AI models)
...
/// Returns true if the AI chat gateway API key is configured.
fn is_chat_enabled() -> bool {
    std::env::var("AGENT_BROWSER_AI_API_KEY").is_ok()
}

/// Returns a JSON response for GET /api/chat/status.
fn chat_status_json() -> String {
...
/// Handle GET /api/models: proxy to the AI Gateway /v1/models endpoint.
async fn handle_models_request(stream: &mut tokio::net::TcpStream) {
...
/// Execute a single agent-browser command for the chat tool and return the result.
...
/// Stream one gateway response, emitting UI Message Stream events.
/// Returns collected tool calls (if any) so the caller can execute them and loop.
async fn stream_gateway_response(
    ...
    while let Some(chunk_result) = byte_stream.next().await {
        ...
        while let Some(newline_pos) = buffer.find('\n') {
            ...
            // Handle tool calls (streamed incrementally by OpenAI)
            if let Some(tcs) = delta.get("tool_calls").and_then(|t| t.as_array()) {
                // Close any open text part before tool calls
                if text_started {
                    let ev = format!("data: {}\n\n", json!({"type":"text-end","id":text_part_id}));
                    let _ = stream.write_all(ev.as_bytes()).await;
                    text_started = false;
...
/// Handle POST /api/chat: proxy to the Vercel AI Gateway with tool-calling
/// support for browser automation. Produces UI Message Stream v1 (SSE).
async fn handle_chat_request(stream: &mut tokio::net::TcpStream, body: &str) {
    let gateway_url = std::env::var("AGENT_BROWSER_AI_GATEWAY_URL")
        .unwrap_or_else(|_| DEFAULT_AI_GATEWAY_URL.to_string())
        .trim_end_matches('/')
        .to_string();
    ...
    // Convert AI SDK UIMessage format to OpenAI messages
    ...
    // Tool-calling loop: up to 10 steps to prevent runaway
    for _step in 0..10 {
        ...
        // Execute each tool call and emit results (within the step boundary)
        ...
        // Close the step AFTER tool execution completes
        let finish_step_ev = "data: {\"type\":\"finish-step\"}\n\n";
        let _ = stream.write_all(finish_step_ev.as_bytes()).await;
    }
